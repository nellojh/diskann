...
thread_local uint64_t thread_count_insert_time=0;
thread_local uint64_t thread_count_pre_time=0;
thread_local uint64_t thread_count_search_time=0;
thread_local uint64_t thread_count_iterate_time=0;
thread_local uint64_t thread_count_erase_time=0;
thread_local uint64_t thread_count_prune_time=0;
thread_local uint64_t thread_count_add_time=0;
thread_local uint64_t thread_count_inter_time=0;

uint64_t final_count_insert_time=0;
uint64_t final_count_pre_time=0;
uint64_t final_count_search_time=0;
uint64_t final_count_iterate_time=0;
uint64_t final_count_erase_time=0;
...
thread_local uint64_t thread_count_find_time;
thread_local uint64_t thread_count_delete_time;
thread_local uint64_t thread_count_pre_delete_time;
thread_local uint64_t thread_count_occ_time;

uint64_t final_count_total_time;
uint64_t final_count_find_delete_time;
uint64_t final_count_find_time;
...
template <typename T, typename TagT, typename LabelT>
std::tuple<uint64_t ,uint64_t,uint64_t> Index<T, TagT, LabelT>::search_for_point_and_prune(int location, uint32_t Lindex,
...
//    std::cout<<pool.size()<<"\n";
    ...
    auto end_count_erase_time=getTimePoint();
    auto count_erase_time=end_count_erase_time-end_count_iterate_time;
    ...
    auto end_count_prune_time=getTimePoint();
    auto count_prune_time=end_count_prune_time-end_count_erase_time;
    assert(!pruned_list.empty());
    assert(_graph_store->get_total_points() == _max_points + _num_frozen_pts);
    return std::tuple<uint64_t ,uint64_t,uint64_t>{count_iterate_time,count_erase_time,count_prune_time};
}
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::occlude_list_new(const std::vector<float> &norm_vec ,const uint32_t location, std::vector<Neighbor> &pool, const float alpha,
                                          const uint32_t degree, const uint32_t maxc, std::vector<uint32_t> &result,
                                          InMemQueryScratch<T> *scratch,
                                          const tsl::robin_set<uint32_t> *const delete_set_ptr)
{
    ...
    while (cur_alpha <= alpha && result.size() < degree)
    {
        ...
        for (auto iter = pool.begin(); result.size() < degree && iter != pool.end(); ++iter)
        {
            ...
            for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++)
            {
                ...
                if (occlude_factor[t] > alpha)
                    continue;
                auto t1 = iter - pool.begin();
                auto tmp_norm_sum=norm_vec[t1]+norm_vec[t];
                auto tmp_norm=tmp_norm_sum*tmp_norm_sum;
                auto tmp_alpha=iter2->distance/tmp_norm;
                if (tmp_alpha > cur_alpha)
                {
                    continue;
                }
                bool prune_allowed = true;
...
template <typename T, typename TagT, typename LabelT>
uint64_t Index<T, TagT, LabelT>::inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, const uint32_t range,
                                          InMemQueryScratch<T> *scratch)
{
    const auto &src_pool = pruned_list;

    assert(!src_pool.empty());
    auto all_count_prune_time=0;
    for (auto des : src_pool)
    {
        ...
        if (prune_needed)
        {
            ...
            auto start_count_prune_time=getTimePoint();
            prune_neighbors(des, dummy_pool, new_out_neighbors, scratch);
            auto end_count_prune_time=getTimePoint();
            auto count_prune_time=end_count_prune_time-start_count_prune_time;
            all_count_prune_time+=count_prune_time;
            {
                LockGuard guard(_locks[des]);
    ...
    return all_count_prune_time;
}

template <typename T, typename TagT, typename LabelT>
uint64_t Index<T, TagT, LabelT>::inter_insert(uint32_t n, std::vector<uint32_t> &pruned_list, InMemQueryScratch<T> *scratch)
{
    return inter_insert(n, pruned_list, _indexingRange, scratch);
}
...
void print_p99(int thread_num,std::vector<std::vector<uint64_t>> &tl_microsecond_latencies) {
    std::vector<uint64_t> microsecond_latencies;
    for (uint64_t t_i = 0; t_i < thread_num; ++t_i) {
    ...
    std::cout << "Sorting Latencies"
              << "\n";
    std::cout << "@p.size: " << microsecond_latencies.size() << std::endl;

    std::sort(microsecond_latencies.begin(), microsecond_latencies.end());

    // 注意输出单位是: 微秒 us

    std::cout << "@min: " << microsecond_latencies[0] << std::endl;
    std::cout << "@median: "
              << microsecond_latencies[microsecond_latencies.size() / 2]
              << std::endl;
    std::cout << "@max: " << microsecond_latencies.back() << std::endl;
    // mean
    uint64_t sum =0 ;
    for (uint64_t s_i =0; s_i < microsecond_latencies.size(); s_i++){
        sum += microsecond_latencies[s_i];
    }
    std::cout << "@mean: " << (sum/microsecond_latencies.size()) << std::endl;


    for (float p = 0.90; p < 1; p += 0.01) {
        std::cout << "@" << p << ": "
                  << (microsecond_latencies[(int)(microsecond_latencies.size() * p)])
                  << std::endl;
    }
    std::cout << "@" << 0.999 << ": "
...
std::vector<std::vector<uint64_t>> tl_count_delete_time;
std::vector<std::vector<uint64_t>> tl_count_pre_delete_time;
std::vector<std::vector<uint64_t>> tl_count_occ_time;
template <typename T, typename TagT, typename LabelT>
inline void Index<T, TagT, LabelT>::process_delete(const tsl::robin_set<uint32_t> &old_delete_set, size_t loc,
                                                   ...
                                                   InMemQueryScratch<T> *scratch)
{
    auto start_count_pre_delete_time=getTimePoint();
    ...
    auto end_count_pre_delete_time=getTimePoint();
...
//    count_600.emplace_back(expanded_nodes_set.size());
    if (modify)
    {
        ...
        else
        {
            ...
            norm_vec.reserve(expanded_nodes_set.size());
            for (auto &ngh : expanded_nodes_set)
            {
            ...
            for (auto &ngh : expanded_nghrs_vec)
            {
                norm_vec.emplace_back(_data_store->get_norm((location_t)ngh.id));
            }
...
//                         &old_delete_set);
            occlude_list_new(norm_vec,(uint32_t)loc, expanded_nghrs_vec, alpha, range, maxc, occlude_list_output, scratch,
                         &old_delete_set);
            std::unique_lock<non_recursive_mutex> adj_list_lock(_locks[loc]);
            _graph_store->set_neighbours((location_t)loc, occlude_list_output);
        }
    }
    auto end_count_occ_time=getTimePoint();
    tl_count_occ_time[t_id].push_back(end_count_occ_time-start_count_occ_time);
}
...
{
    auto start_count_total_time=getTimePoint();
    if (!_enable_tags)
    ...
    tl_count_delete_time.resize(num_threads);
    tl_count_pre_delete_time.resize(num_threads);
    tl_count_occ_time.resize(num_threads);
...
//    outfile_2<<i<<"_"<<_graph_store->get_neighbours(i).size()<<"\n";
//    //        for(auto j=0;j<_graph_store->get_neighbours(i).size();j++){
//    //
//    //        }
//
//}
//outfile_2.close();
    for (int64_t loc = 0; loc < (int64_t)_max_points; loc++)
    {
        if (old_delete_set->find((uint32_t)loc) == old_delete_set->end() && !_empty_slots.is_in_set((uint32_t)loc))
        {
            ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
            auto scratch = manager.scratch_space();
            auto start_count_delete_time=getTimePoint();
            process_delete(*old_delete_set, loc, range, maxc, alpha, scratch);
            auto end_count_delete_time=getTimePoint();
            int t_id = omp_get_thread_num(); // 线程id
            tl_count_delete_time[t_id].push_back(end_count_delete_time-start_count_delete_time);
            num_calls_to_process_delete += 1;
        }
    }
//    print_p99(num_threads,tl_microsecond_latencies);
    ...
    auto end_count_find_delete_time=getTimePoint();
    final_count_find_delete_time=end_count_find_delete_time-start_count_find_delete_time;
    std::unique_lock<std::shared_timed_mutex> tl(_tag_lock);
    ...
    auto end_count_total_time=getTimePoint();
    final_count_total_time=end_count_total_time-start_count_total_time;
    //std::cout<<"\n\n               delete part\n\n";
    double duration = timer.elapsed() / 1000000.0;
    diskann::cout << " done in " << duration << " seconds." << std::endl;
    final_count_delete_time=0;
    final_count_pre_delete_time=0;
    final_count_occ_time=0;
    for (uint64_t t_i = 0; t_i < num_threads; ++t_i) {
        for(auto it:tl_count_delete_time[t_i]){
            final_count_delete_time+=it;
        }
        for(auto it:tl_count_pre_delete_time[t_i]){
            final_count_pre_delete_time+=it;
        }
        for(auto it:tl_count_occ_time[t_i]){
            final_count_occ_time+=it;
        }
    }
//    std::cout<<"\n*****************Delete part total time*********************\n";
//    std::cout<<"final_count_total_time:"<<final_count_total_time<<"\n";
//    std::cout<<"final_count_find_delete_time:"<<final_count_find_delete_time<<"\n";
//    std::cout<<"final_count_find_time:"<<final_count_find_time<<"\n";
//    std::cout<<"final_count_delete_time:"<<final_count_delete_time<<"\n";
//    std::cout<<"final_count_pre_delete_time:"<<final_count_pre_delete_time<<"\n";
//    std::cout<<"final_count_occ_time:"<<final_count_occ_time<<"\n";
//    std::cout<<"\n*****************Delete part P99*********************\n";
//    std::cout<<"\n*****************tl_count_delete_time*********************\n";
//    print_p99(num_threads,tl_count_delete_time);
//    std::cout<<"\n*****************tl_count_pre_delete_time*********************\n";
//    print_p99(num_threads,tl_count_pre_delete_time);
//    std::cout<<"\n*****************tl_count_occ_time*********************\n";
//    print_p99(num_threads,tl_count_occ_time);
...
{
    auto start_count_insert_time=getTimePoint();
    assert(_has_built);
    if (tag == 0)
    {
    ...
    auto end_count_pre_time=getTimePoint();
    auto count_pre_time=end_count_pre_time-start_count_insert_time;
    // Find and add appropriate graph edges
    ScratchStoreManager<InMemQueryScratch<T>> manager(_query_scratch);
    auto scratch = manager.scratch_space();
    std::vector<uint32_t> pruned_list; // it is the set best candidates to connect to this point
    auto start_count_search_time=getTimePoint();
    std::tuple<uint64_t ,uint64_t,uint64_t> t_time;
    if (_filtered_index)
    {
    ...
    {
        t_time=search_for_point_and_prune(location, _indexingQueueSize, pruned_list, scratch);
    }
    auto end_count_search_time=getTimePoint();
    auto count_search_time=end_count_search_time-start_count_search_time;
    ...
    auto end_count_add_time=getTimePoint();
    auto count_add_time=end_count_add_time-end_count_search_time;
    auto all_count_prune_time=inter_insert(location, pruned_list, scratch);
    auto end_count_insert_time=getTimePoint();
    auto count_inter_time=end_count_insert_time-end_count_add_time;
    auto count_insert_time=end_count_insert_time-start_count_insert_time;
//    std::cout<<"\n**********************************\n";
//    std::cout<<"count_insert_time:"<<count_insert_time<<"\n";
//    std::cout<<"count_pre_time:"<<count_pre_time<<"\n";
//    std::cout<<"count_search_time:"<<count_search_time<<"\n";
//    std::cout<<"count_iterate_time:"<<std::get<0>(t_time)<<"\n";
//    std::cout<<"count_erase_time:"<<std::get<1>(t_time)<<"\n";
//    std::cout<<"count_prune_time:"<<std::get<2>(t_time)<<"\n";
//    std::cout<<"count_add_time:"<<count_add_time<<"\n";
//    std::cout<<"count_inter_time:"<<count_inter_time<<"\n";
//    std::cout<<"\n**********************************\n";

    thread_count_insert_time+=count_insert_time;
    thread_count_pre_time+=count_pre_time;
    thread_count_search_time+=count_search_time;
    thread_count_iterate_time+=std::get<0>(t_time);
    thread_count_erase_time+=std::get<1>(t_time);
    thread_count_prune_time+=std::get<2>(t_time);
    thread_count_add_time+=count_add_time;
    thread_count_inter_time+=count_inter_time;


    final_count_insert_time+=count_insert_time;
    final_count_pre_time+=count_pre_time;
    final_count_search_time+=count_search_time;
    final_count_iterate_time+=std::get<0>(t_time);
    final_count_erase_time+=std::get<1>(t_time);
    final_count_prune_time+=std::get<2>(t_time);
    final_count_add_time+=count_add_time;
    final_count_inter_time+=count_inter_time;
    return 0;
}
