                       ...
                       size_t aligned_dim, std::vector<std::vector<LabelT>> &pts_to_labels)
{
    try
    {
        std::vector<std::vector<uint64_t>> tl_count_insert_time;
        std::vector<std::vector<uint64_t>> tl_count_pre_time;
        std::vector<std::vector<uint64_t>> tl_count_search_time;
        std::vector<std::vector<uint64_t>> tl_count_iterate_time;
        std::vector<std::vector<uint64_t>> tl_count_erase_time;
        std::vector<std::vector<uint64_t>> tl_count_prune_time;
        std::vector<std::vector<uint64_t>> tl_count_add_time;
        std::vector<std::vector<uint64_t>> tl_count_inter_time;

        tl_count_insert_time.resize(insert_threads);
        tl_count_pre_time.resize(insert_threads);
        tl_count_search_time.resize(insert_threads);
        tl_count_iterate_time.resize(insert_threads);
        tl_count_erase_time.resize(insert_threads);
        tl_count_prune_time.resize(insert_threads);
        tl_count_add_time.resize(insert_threads);
        tl_count_inter_time.resize(insert_threads);

        thread_count_insert_time=0;
        thread_count_pre_time=0;
        thread_count_search_time=0;
        thread_count_iterate_time=0;
        thread_count_erase_time=0;
        thread_count_prune_time=0;
        thread_count_add_time=0;
        thread_count_inter_time=0;

        final_count_insert_time=0;
        final_count_pre_time=0;
        final_count_search_time=0;
        final_count_iterate_time=0;
        final_count_erase_time=0;
...
//#pragma omp parallel for num_threads((int32_t)insert_threads) schedule(dynamic) reduction(+ : num_failed)
        for (int64_t j = start; j < (int64_t)end; j++)
        {
            ...
            tl_count_insert_time[t_id].push_back(thread_count_insert_time);
            tl_count_pre_time[t_id].push_back(thread_count_pre_time);
            tl_count_search_time[t_id].push_back(thread_count_search_time);
            tl_count_iterate_time[t_id].push_back(thread_count_iterate_time);
            tl_count_erase_time[t_id].push_back(thread_count_erase_time);
            tl_count_prune_time[t_id].push_back(thread_count_prune_time);
            tl_count_add_time[t_id].push_back(thread_count_add_time);
